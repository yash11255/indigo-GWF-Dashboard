...
const COLORS = ['#003B8E', '#0066CC', '#60A5FA', '#CBD5E1'];
...
export default function GenderChart({ data }) {
  ...
  const enriched = data.map(d => ({ ...d, total, name: d.gender }));
      ...
      <ResponsiveContainer width="100%" height={160}>
        <PieChart>
          <Pie data={enriched} cx="50%" cy="50%" innerRadius={42} outerRadius={70}
            dataKey="count" nameKey="gender" paddingAngle={3} strokeWidth={0}>
      ...
      <div className="flex flex-wrap justify-center gap-x-4 gap-y-1.5 mt-2">
        {enriched.map((d, i) => (
          <div key={d.gender} className="flex items-center gap-1.5">
            ...
            <span className="text-slate-500 text-xs">{d.gender}</span>
